...
# Spectral GNN model
# ------------------------
class DrugSpectralGNN(nn.Module):
    def __init__(self, in_dim, K=3):
        super().__init__()
        self.conv1 = ChebConv(in_dim, 32, K=K)
        self.conv2 = ChebConv(32, 32, K=K)
...
model = DrugSpectralGNN(drug_graphs[0].x.shape[1], K=3)
...
for epoch in range(50):
    ...
    if (epoch + 1) % 10 == 0:
        print(f"[Spectral-GNN] Epoch {epoch+1:03d} | Loss: {loss.item():.4f}")
